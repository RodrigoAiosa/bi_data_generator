...
def _dash_telecom(tabelas: dict[str, pd.DataFrame]) -> None:
    ...
    if has_custo:
        metrics.append(("Custo Total", f"R$ {fato['custo'].sum():,.2f}", "chamadas tarifadas"))
    else:
        metrics.append(("Dados MB", f"{fato['dados_mb'].mean():.1f} MB", "médio por chamada"))
    
    if has_qualidade:
        metrics.append(("Qualidade Média", f"{fato['qualidade_sinal'].mean():.1f}/5", "score de sinal"))
    else:
        metrics.append(("Qualidade dBm", f"{fato['qualidade_dbm'].mean():.0f} dBm", "sinal médio"))
    ...
    fig_dia = px.line(by_dia, x="dia", y="count",
                      labels={"dia": "", "count": "Número de Chamadas"})
...
def _dash_industria(tabelas: dict[str, pd.DataFrame]) -> None:
    ...
    refugo_total = fato[refugo_col].sum() if refugo_col == "refugo_pct" else fato[refugo_col].sum()
    refugo_pct = (refugo_total / qtd_total * 100) if qtd_total > 0 else 0
    oee_medio = fato[oee_col].mean() * 100
    custo_total = fato[custo_col].sum()
    
    _section("Visão Geral")
    _kpi_row([
        ("Produção Total", f"{qtd_total:,.0f}", "unidades"),
        ("Taxa de Refugo", f"{refugo_pct:.2f}%", "perda"),
        ("OEE Médio",      f"{oee_medio:.1f}%", "eficiência global"),
        ("Custo Total",    f"R$ {custo_total:,.0f}", "produção"),
    ])

    # Produção por máquina (se existir a coluna)
    if "id_maquina" in fato.columns:
        ...
        fig_maq = px.bar(by_maq, x="id_maquina", y=qtd_col,
                         labels={qtd_col: "Quantidade Produzida", "id_maquina": "Máquina"})
        _base_layout(fig_maq, "Produção por Máquina")
        _chart_row([(fig_maq, 1)])
    else:
        st.info("Dados de máquina não disponíveis para esta base.")
...
def _dash_turismo(tabelas: dict[str, pd.DataFrame]) -> None:
    ...
    fig_mes = px.area(by_mes, x="mes", y="valor_pago",
                      labels={"mes": "", "valor_pago": "Receita (R$)"})
    fig_mes.update_traces(line_color=_ACCENT, fillcolor="rgba(167,139,250,0.15)")
    _base_layout(fig_mes, "Receita Mensal")
    ...
    if "id_destino" in fato.columns:
        ...
        fig_pais = px.bar(by_pais, x="pais", y="valor_pago", 
                          labels={"valor_pago": "Receita (R$)", "pais": ""},
...
def _dash_imobiliario(tabelas: dict[str, pd.DataFrame]) -> None:
    ...
    fig_mes = px.area(by_mes, x="mes", y="valor_final",
                      labels={"mes": "", "valor_final": "Volume (R$)"})
    fig_mes.update_traces(line_color=_ACCENT, fillcolor="rgba(167,139,250,0.15)")
    ...
    fig_tipo = px.pie(by_tipo, names="tipo", values="valor_final",
                      hole=0.55, color_discrete_sequence=_PALETTE)
    fig_tipo.update_traces(textfont_color="#e2e8f0")
...
def _dash_seguros(tabelas: dict[str, pd.DataFrame]) -> None:
    ...
    fig_mes = px.area(by_mes, x="mes", y="valor_premio",
                      labels={"mes": "", "valor_premio": "Prêmios (R$)"})
    fig_mes.update_traces(line_color=_ACCENT, fillcolor="rgba(167,139,250,0.15)")
    ...
    fig_tipo = px.bar(by_tipo, x="valor_premio", y="tipo", orientation="h",
                      labels={"valor_premio": "Prêmios (R$)", "tipo": ""},
...
def _dash_construcao(tabelas: dict[str, pd.DataFrame]) -> None:
    ...
    fig_proj = px.bar(by_proj, x="nome", y="custo_real",
                      labels={"custo_real": "Custo (R$)", "nome": ""},
